...
internal object DecodeHintManager {
        ...
        for (hintType in DecodeHintType.values()) {
            ...
            if (hintType.valueType == IntArray::class.java) {
                ...
                for (i in values.indices) {
                    try {
                        array?.set(i, Integer.parseInt(values[i]))
                    } catch (ignored: NumberFormatException) {
                        Log.w(TAG, "Skipping array of integers hint " + hintType + " due to invalid numeric value: '" + values[i] + '\''.toString())
                        array = null
                        break
                    }
